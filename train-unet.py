...
###	imgs_mask_train = np.load('train_label_SAT.npy')
	imgs_mask_train = np.load('train_label_VAT.npy')

	imgs_train = imgs_train.reshape(912,512,512,1)
	imgs_mask_train = imgs_mask_train.reshape(912,512,512,1)
...
###	model_checkpoint = ModelCheckpoint('weights_SAT.h5', monitor='val_loss', save_best_only=True)
	model_checkpoint = ModelCheckpoint('weights_VAT.h5', monitor='val_loss', save_best_only=True)
##	tb_cb = TensorBoard(log_dir=log_filepath, write_images=False, histogram_freq=1, write_graph=True)
	...
	print('-'*30)
    #训练网络
	model.fit(imgs_train, imgs_mask_train, batch_size=1, epochs=50, verbose=1, shuffle=True,validation_split=0.01,callbacks=[model_checkpoint])
...
#	np.save('imgs_mask_train_SAT.npy', imgs_mask_train)
#	np.save('imgs_mask_train_VAT.npy', imgs_mask_train)

#	print('-' * 30)
#	print('Saving predicted masks to files...')
#	print('-' * 30)

#	imgs_id_test = np.ndarray((100, ), dtype=np.int32)
#	for i in range(100):
#		imgs_id_test[i] = i+1
